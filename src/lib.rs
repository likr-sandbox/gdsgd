...
pub fn multi_source_shortest_path<N, E, Ty: EdgeType, Ix: IndexType>(
...
pub fn power_iteration(a: &Array2<f32>) -> (f32, Array1<f32>) {
    let n = a.shape()[0];
...
pub fn eigendecomposition(a: &Array2<f32>, k: usize) -> (Array1<f32>, Array2<f32>) {
    let n = a.shape()[0];
    let mut b = a.clone();
    let mut e = Array1::zeros(k);
    let mut v = Array2::zeros((n, k));
    let (ei, vi) = power_iteration(&b);
    e[0] = ei;
...
pub fn classical_mds<N, E, Ty: EdgeType, Ix: IndexType>(
    graph: &Graph<N, E, Ty, Ix>,
    ...
    let (e, v) = eigendecomposition(&b, 2);
...
pub fn pivot_mds<N, E, Ty: EdgeType, Ix: IndexType, R: Rng>(
    graph: &Graph<N, E, Ty, Ix>,
    k: usize,
    rng: &mut R,
) -> HashMap<NodeIndex<Ix>, (f32, f32)> {
    ...
    let ct_c = c.t().dot(&c);
    let (e, v) = eigendecomposition(&ct_c, 2);
    let xy = v.dot(&Array2::from_diag(&e.mapv(|v| v.sqrt())));
    let xy = c.dot(&xy);
    let mut pos = HashMap::new();
    for (i, u) in graph.node_indices().enumerate() {
...
pub fn generate_graph<R: Rng>(n: usize, p: f32, rng: &mut R) -> Graph<(), (), Undirected> {
